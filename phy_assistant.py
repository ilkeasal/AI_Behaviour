...
def assess_concepts_prompt(concept_name, user_prompt, chat_history):
   ...
   formatted_prompt = f"""Respond to the  {user_prompt} with a friendly and relevant acknowledgement. If the users' input includes a question or request for clarification, address it directly. Additionally, if the user's input raises areas that could benefit from further exploration, such as challenges, concerns, or unclear aspects, ask at most one relevant and neutral follow-up question, but only if it is strictly necessary based on the user's input. Avoid asking unnecessary follow-up questions when the input does not indicate a need for further exploration. If a follow-up question is needed, ensure a smooth transition from it to the part of the response that steers the conversation toward discussing aspects related to {concept_name} about physical activity. Regardless of whether a follow-up question is asked, always naturally steer the conversation toward {concept_name}. {concept_name} {concept_definitions_dict[concept_name]}. When steering the conversation, ask exactly one direct and neutral question aimed at understanding the user's current behaviour or thoughts about {concept_name}. This question must assess their current level on {concept_name}, without making any assumptions about what the user does/thinks or does not do/think. Do not make assumptions about the user based solely on their input. Ensure that your response transitions smoothly from {user_prompt} and is open-ended to encourage discussion. Do not include any explanations, reflections, or commentary about the purpose, structure, or intent of your response, either implicitly or explicitly. Use relevant aspects from the chat history ({chat_history}) to maintain context but do not repeat previous responses verbatim. Each response should contribute new insights or questions to the conversation without breaking the flow or including unnecessary meta-information. Keep your responses concise, ideally under 250 tokens, while maintaining a complete thought."""
...
def submit_submit_function():
   ...
   elif total_activity_levels >=150 and strength_input<2:
...
def post_survey_submit():
   ...
   st.session_state.log_buffer.write(f"Q1 : The chatbot provided me a customized conversation.: {question1}\n")
   st.session_state.log_buffer.write(f"Q2 : I am satisfied with the advice given to me by this chatbot.: {question2}\n")
...
def continue_the_conversation():
   continue_prompt = f"Respond to the user's input : {user_prompt} with a friendly and relevant acknowledgement. If the user's input includes a question or request for clarification, address it directly. If the input is related to physical activity, ask only one follow up question to continue the conversation. If the input is unrelated to physical activity, acknowledge it and then steer conversation naturally towards discussing physical activity by asking only one question. Do not make assumptions about the user solely based on their input. Use relevant parts of the chat history : {chat_history} for context, but do not repeat yourself word-for-word. Make sure your response builds on the past conversations and adds new insights. Keep your response concise ideally under 250 tokens, while maintaining a complete thought."
   completion = client.chat.completions.create(model="gpt-4o-mini",messages=[{
      "role":"assistant",
...
if st.session_state.start_experiment =="consent":
   st.markdown("""Welcome, and thank you for participating in this experiment. This experiment is part of a research project conducted by the _**AI & Behaviour group**_ at the _**Vrije Universiteit Amsterdam**_. It involves _**interacting with a chatbot in a conversation focused on physical activity**_. _**It consists of three parts**_. _**In the first part, you will answer a few brief questions about yourself, such as demographics, prior chatbot experience, physical activity levels, and motivation to be physically active. This part will take about 2-3 minutes to complete.**_\n
As the main phase of this experiment, _**the second part involves a conversation with a chatbot, which is expected to last at least 15 minutes, although you may choose to continue for a longer duration**_. During the conversation, a _**‘Stop the conversation’**_ button will appear at a certain point, giving you the option to end the conversation. _**Clicking this button will take you to the final part of the experiment, consisting of post-survey questions, which will take approximately 2-3 minutes to complete.**_ If you wish to continue the conversation, no action is required-simply keep interacting with the chatbot. _**The ‘Stop the conversation’ button will become available at specific times, allowing you to stop the conversation later if you choose.**_ If the button is not clicked, the conversation will continue indefinitely.\n
While interacting with the chatbot, you will notice the word _**‘Running’**_ displayed in the top-right corner after you provide an input. This indicates that the chatbot is processing your input. The ‘Running’ indicator will disappear once the chatbot has completed its response. _**Please wait for the chatbot to finish generating its response before entering a new input.**_ Similarly, during the first and final parts of the experiment, the ‘Running’ indicator will signify that your answer is being processed. _**Please wait for the ‘Running’ indicator to disappear before proceeding to the next question.**_ After completing the final question and once the ‘Running’ indicator disappears, you may click the _**‘Submit’**_ button.\n
Please be aware that while your conversation with the chatbot will be saved for research purposes, your privacy is our priority. _**The conversation will not be linked to your identity.**_ Instead, you will be assigned a randomly generated ID, and all data will be associated with this ID only. We want to ensure that you feel comfortable throughout the process. You are free to stop participating in the experiment at any time by simply closing your browser, without needing to provide any reason. If you choose to do so, all data associated with your session will be removed. For participants who continue with the experiment until the end, the information collected will be used solely for the purpose of this study. \n
_**Important Note :**_\n 
_**Your participation in this study is essential to our research, and we would greatly appreciate it if you complete the experiment in its entirety. Once the experiment is finished, a message saying 'Thank you for participating!' will appear on the screen. Please note that if you close your browser before completion, all associated data–including chatbot conversations and your respones to the pre- and post-conversation questions–will be permanently deleted.**_\n
   """)
   st.markdown("_**I have read and understood the information provided about the experiment and agree to participate voluntarily.**_")
...
if st.session_state.start_experiment=="pre-survey":
   ...
   hours_input = st.number_input("On average, how many minutes per day do you engage in physical activity at this level?",min_value=0,max_value=240)
   ...
   importance_input = survey.select_slider("How important is physical activity in your life? (The options are : Not important, Slightly important, Moderately important, Important, and, Very important)", options=["Not important","Slightly important","Moderately important","Important","Very important"], id= "Q1")
   motivation_input = survey.select_slider("How motivated are you to maintain or enhance your physical activity level? (The options are : Not motivated, Slightly motivated, Moderately motivated, Motivated, and, Very motivated.)", options = ["Not motivated","Slightly motivated","Moderately motivated","Motivated","Very motivated"],id="Q2")
   support_input = survey.select_slider("How do you feel about receiving support to become more physically active? (The options are : Strongly opposed, Somewhat opposed, Neutral, Somewhat open, and, Strongly open.)", options = ["Strongly opposed","Somewhat opposed","Neutral","Somewhat open","Strongly open"],id="Q3")
   #st.markdown(f"""
          ...
          if "none of the above" in construct_prompt:
              ...
              try:
                  ...
                  completion = client.chat.completions.create(
                      ...
                      messages=[
                          {"role": "assistant",
                           "content": f"""Among all the concepts mentioned in {variables}, decide which one : {user_prompt} relates to. Only return the name of the concept. If the {user_prompt} does not directly relate to physical activity or explicitly address any of the listed concepts, includes general daily tasks like calling someone or waking up, or consists of a question, or seeks clarification about a concept, return 'None of the above.' If the input explicitly asks for a definition, explanation, or meaning of a concept (e.g., 'What is coping strategies?', 'Can you explain action planning?' etc.. ), categorize it as 'None of the above.' Only statements that clearly provide specific information about physical activity or challenges related to physical activity should be categorized. If the input implicitly or explicitly points to a difficulty, change, or adjustment related to physical activity, categorize it under the concept most relevant to the user's situation, as long as it clearly relates to one of the listed concepts. Prioritize categorizing any difficulties, challenges, or obstacles related to physical activity. If the user mentions any problems or struggles with engaging in physical activity-such as interruptions to a regular routine, lack of tine, or motivation issues- character it under the concept most related to the problem they are facing. However, only select from the listed concepts. Do not create new categories or concepts that are not explicitly mentioned in the list. If the difficulty does align well with any of the listed concepts, return 'None of the above'. If no problems are mentioned, categorize the input based on positive aspects of physical activity, but only if the positive aspects are related to any of the listed concepts. If the positive aspects are not relevant to any of the listed concepts, categorize it as 'None of the above.' When determining the concept take the {user_prompt} and chat history : {st.session_state.messages} into account. Check the chat history to determine if the user's input reflects a positive or neutral acknowledgement of a previously given recommendation, if so, categorize it as 'None of the above'. Attitude refers to beliefs about how harmful, healthy, enjoyable, and boring physical activity is perceived. Self-efficacy refers to people's belief in their ability to engage in physical activity to achieve their fitness goals, even in the face of challenges. Exercise self identity refers to the extent to which an individual sees exercise as a fundamental part of who they are. Action planning refers to the process of making detailed plans about when, where, and how often one engages in physical activity. Coping strategies refers to an individual's ability to manage tempting situations and cues that could disrupt their physical activity routine. Intentions refers to the amount of physical activity an individual intends to do. Habit strength refers to the degree of automaticity and consistency in an individual's physical activity behaviour often developed through a history of repeated actions. Social support non-family refers to support for physical activity from friends, acquaintances, or co-workers. Social support family refers to support for physical activity from family. Mood refers to temporary state of mind defined by feelings and dispositions. Self monitoring refers to an individual's ability to regularly track and evaluate their physical activity levels. Social norms refers to the cultural and environmental influences shaping how individuals perceive and engage in physical activity."""}
                      ]
                  )
                  construct_prompt = completion.choices[0].message.content
                  construct_prompt = construct_prompt.lower()
                 ...
                 if st.session_state.random_asked_concept == construct_prompt: 
                    ...
                    if "Low" in construct_name_level or "low" in construct_name_level:
                         ...
                         if not st.session_state.unstable_concept_dict:  # if the dictionary is empty, do not even create users_level and just give advice then empty the unstable concept dict
                             ...
                             st.session_state.log_buffer.write("\n")
                             #st.write(st.session_state.problematic_sentence)
                             st.session_state.log_buffer.write(f"USERS LEVEL : {st.session_state.users_level}\n")
                             ...
                             assess_concepts_prompt(concept_name=st.session_state.last_asked_key, user_prompt=user_prompt,
                                                    chat_history=chat_history)
                             st.session_state.random_asked_concept = st.session_state.last_asked_key
   ...
   if st.session_state.start_experiment == "experiment":
      ...
      if user_prompt:
         completion = client.chat.completions.create(
            ...
            messages=[
               {"role":"assistant",
                "content":f"Respond to the user's input : {user_prompt} with a friendly and relevant acknowledgement. If the user's input includes a question or request for clarification, address it directly. Additionally, if the user's input raises areas that could benefit from further exploration, such as challenges, concerns, or unclear aspects, ask at most one relevant and neutral follow-up question, but only if it is strictly necessary based on the user's input. Avoid asking unnecessary follow-up questions when the input does not indicate a need for further exploration. Then naturally steer the conversation towards physical activity by asking only one direct and neutral question. Do not make assumptions about the user solely based on their input. If you believe you have understood the user's situation related to physical activity, offer advice if necessary. If you give an advice, relate your advice to the user's input to show you are listening and phrase your recommendation in a unique, and varied language to keep the conversation fresh and engaging. After providing your advice, ask the user what they think about trying the advice or how they might incorporate it into their routine. Use relevant aspects from the chat history : {chat_history} to maintain context, but do not repeat previous responses verbatim. Make sure your response builds on the past conversations and adds new insights. Do not include any explanations, reflections, or commentary about the purpose, structure, or intent of your response, either implicitly or explicitly. Keep your response concise ideally under 250 tokens, while maintaining a complete thought."
...
if st.session_state.start_experiment == "post-survey":
   ...
   question1 = survey.select_slider("Q1 : The chatbot provided me a customized conversation.", options=["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id="Q12")
   question2 = survey.select_slider("Q2 : I am satisfied with the advice given to me by this chatbot.",options=["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id="Q13")
   question3 = survey.select_slider("Q3 : The chatbot used my thoughts and habits regarding physical activity to provide personalized advice.",options=["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id="Q14")
   question4 = survey.select_slider("Q4 : I would recommend this chatbot to a friend who wants help with improving their physical activity.",options =["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id = "Q15")
   question5 = survey.select_slider("Q5 : The chatbot recognized me as a unique individual.",options =["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id = "Q16")
   question6 = survey.select_slider("Q6 : If in the future I need support in physical activity, I would be interested in a further dialogue with this chatbot.",options =["Strongly Disagree","Disagree","Somewhat Disagree","Neutral","Somewhat Agree","Agree","Strongly Agree"],id = "Q17")
